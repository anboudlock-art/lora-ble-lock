...
//ÓÃ»§Ìí¼Ó,ÏÂÃæµÄ¼Ó½âÃÜ¿ÉÖ±½Ó¶Ô½Ócc2541¿âµÄaes

/*
Key:ÃÜÔ¿
CipherContent£ºÃ÷ÎÄ(Ðè¼ÓÃÜµÄÊý¾Ý)
PlainContent£º °µÎÄ(¼ÓÃÜºóµÄÊý¾Ý)
*/
int my_aes_encrypt(const unsigned char* Key, unsigned char* CipherContent, unsigned char* PlainContent)
{
	unsigned int roundkeys[68];
	unsigned char numberOfRound = 0;
	aes_key_de_schedule(Key, 16 * 8, roundkeys, &numberOfRound);
	
	aes_crypt_ecb(ENCRYPT, roundkeys, &numberOfRound, &CipherContent[0], &PlainContent[0]);
...
/*
Key:ÃÜÔ¿
CipherContent£ºÃ÷ÎÄ(Ðè½âÃÜµÄÊý¾Ý)
PlainContent£º °µÎÄ(½âÃÜºóµÄÊý¾Ý)
*/
int my_aes_decrypt(const unsigned char* Key, unsigned char* CipherContent, unsigned char* PlainContent)
{
	...
	aes_crypt_ecb(DECRYPT, roundkeys, &numberOfRound, &CipherContent[0], &PlainContent[0]);
